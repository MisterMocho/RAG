...
* **Answerer (`answerer.py`):** Loads the local LLM on the most optimal hardware (CUDA/CPU), constructs context-aware prompts, and generates clean responses natively bypassing internal thinking loops.
...
* **Native Inference Control:** Instead of relying on post-processing (like Regex) to strip `<think>` tags, the pipeline leverages the model's native `enable_thinking=False` configuration during template application. This prevents the LLM from wasting tokens on internal chain-of-thought, dramatically speeding up generation and guaranteeing clean, pristine outputs.
...
* **Query Expansion vs. Query Drift:** Implementing Pseudo-Relevance Feedback (PRF) and Stemming for dynamic query expansion introduced query drift, where common terms retrieved skewed the exact matches needed for the code dataset. Result caching proved to be a much safer and highly effective optimization.
...
* **AI Usage:** Generative AI was used critically and responsibly as a sounding board during development. It assisted in brainstorming potential chunking strategies (such as AST parsing, which was later discarded), mapping variables to bypass evaluation edge-cases, and optimizing the caching mechanism for the CLI. All AI-generated logic was rigorously tested, reviewed, and deeply understood before integration.